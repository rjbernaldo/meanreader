...
var Article = React.createClass({displayName: 'Article',
  ...
  componentDidMount: function() {
    ...
    var test = this.props.data.title.replace("'", "").replace(/\s/g, "").replace(":", "").replace(" ", "")
    console.log(test);
    var canvas = $('#' + test)[0];
    ...
    return (
      React.DOM.div({className: "article"}, 
        ...
        React.DOM.div({style: articleStyle }, 
          ...
          React.DOM.p(null, React.DOM.span(null, React.DOM.canvas({id:  this.props.data.title.replace("'", "").replace(/\s/g, "").replace(":", "").replace(" ", "") }),  this.state.date, " ago")), 
