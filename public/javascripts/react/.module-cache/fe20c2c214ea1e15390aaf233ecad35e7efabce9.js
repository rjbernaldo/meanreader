...
var Article = React.createClass({displayName: 'Article',
  ...
  componentDidMount: function() {
    ...
    var test = this.props.data.title.replace("'", "").replace(/\s/g, "").replace(":", "")
    var canvas = $('#' + test)[0];
    ...
    return (
      React.DOM.div({className: "article"}, 
        ...
        React.DOM.div({style: articleStyle }, 
          React.DOM.h1(null,  this.props.data.title.replace("'", "").replace(":", "") ), 
          React.DOM.p(null,  this.props.data.author), 
          React.DOM.p(null, React.DOM.span(null, React.DOM.canvas({id:  this.props.data.title.replace("'", "").replace(/\s/g, "") }),  this.state.date, " ago")), 
